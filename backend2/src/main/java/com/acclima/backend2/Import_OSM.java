...
public class Import_OSM extends groovy.lang.Script {
    public static void main(java.lang.String[] args) {
        new com.acclima.backend2.Import_OSM(new groovy.lang.Binding(args)).run();
    }
...
// main function of the script
    ...
    public void loadOSMData(org.openstreetmap.osmosis.core.task.v0_6.RunnableSource reader, groovy.sql.Sql sql, java.lang.String tableName) {
        java.lang.Integer srid = 3857;// TODO

        org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger("com.acclima.backend2");
        logger.info("Start : Get Buildings from OSM");
        logger.info("inputs {}");

        com.acclima.backend2.OsmHandler handler = new com.acclima.backend2.OsmHandler(logger, false, false, false, false);
        reader.setSink(handler);
        reader.run();

        logger.info("OSM Read done");

        sql.execute("DROP TABLE IF EXISTS ?", new java.util.ArrayList<java.lang.String>(java.util.Arrays.asList(tableName)));
        sql.execute("CREATE TABLE ? ( \n        ID_WAY integer PRIMARY KEY, \n        THE_GEOM geometry,\n        HEIGHT real\n    );", new java.util.ArrayList<java.lang.String>(java.util.Arrays.asList(tableName)));

        for (com.acclima.backend2.Building building : handler.buildings) {
            ...
            sql.execute("INSERT INTO ? VALUES (?, ST_MakeValid(ST_SIMPLIFYPRESERVETOPOLOGY(ST_Transform(ST_GeomFromText(?, 4326), ?),0.1)), ?)", new java.util.ArrayList<java.lang.Object>(java.util.Arrays.asList(tableName, building.getId(), building.getGeom(), srid, building.getHeight())));
        }
        ...
        for (com.acclima.backend2.Road road : handler.roads) {
            if (road.getGeom().invokeMethod("isEmpty", new java.lang.Object[0]).asBoolean()) {
                continue;
            }

            java.lang.String query = "INSERT INTO ROADS(ID_WAY, THE_GEOM, TYPE, LV_D, LV_E, LV_N, HV_D, HV_E, HV_N, LV_SPD_D, LV_SPD_E, LV_SPD_N, HV_SPD_D, HV_SPD_E, HV_SPD_N, PVMT) VALUES (?, st_setsrid(st_updatez(ST_precisionreducer(ST_SIMPLIFYPRESERVETOPOLOGY(ST_TRANSFORM(ST_GeomFromText(?, 4326), ?),0.1),1), 0.05), ?),?,?,?,?,?,?,?,?,?,?,?,?,?,?);";
            sql.execute(query, new java.util.ArrayList<java.lang.Object>(java.util.Arrays.asList(road.getId(), road.getGeom(), srid, srid, road.getType(), road.getNbLV("d"), road.getNbLV("e"), road.getNbLV("n"), road.getNbHV("d"), road.getNbHV("e"), road.getNbHV("n"), com.acclima.backend2.Road.getSpeed().get(road.getCategory()), com.acclima.backend2.Road.getSpeed().get(road.getCategory()), com.acclima.backend2.Road.getSpeed().get(road.getCategory()), java.lang.Math.min(90, com.acclima.backend2.Road.getSpeed().get(road.getCategory())), java.lang.Math.min(90, com.acclima.backend2.Road.getSpeed().get(road.getCategory())), java.lang.Math.min(90, com.acclima.backend2.Road.getSpeed().get(road.getCategory())), "NL08")));
        }
        ...
        for (com.acclima.backend2.Ground ground : handler.grounds) {
            if (ground.getPriority() == 0) {
                continue;
            }

            if (ground.getGeom().invokeMethod("isEmpty", new java.lang.Object[0]).asBoolean()) {
                continue;
            }
            ...
            sql.execute("INSERT INTO GROUND (ID_WAY, THE_GEOM, PRIORITY, G) VALUES (?, ST_Transform(ST_GeomFromText(?, 4326), ?), ?, ?)", new java.util.ArrayList<java.lang.Object>(java.util.Arrays.asList(ground.getId(), ground.getGeom(), srid, ground.getPriority(), ground.getCoeff_G())));
        }
    ...
    public void loadOSMData(org.openstreetmap.osmosis.core.task.v0_6.RunnableSource reader, groovy.sql.Sql sql) {
        loadOSMData(reader, sql, "MAP_BUILDINGS_GEOM");
    }

    public Import_OSM(groovy.lang.Binding binding) {
        super(binding);
    }
