...
def exec(Connection connection, input) {
    ...
    if (!ignoreBuilding) {
        String tableName = "MAP_BUILDINGS_GEOM_" + cid;
        ...
        sql.execute "insert into $tableName values (select * from $buildingsTable)"; // insert in n^2 :/
        ...
        String tmpBuildTrunc = "tmp_buildings_truncated_" + cid
        String tmpRelBuild = "tmp_relation_buildings_" + cid
        String buildingsIdx = "buildings_index_" + cid

        sql.execute """
            CREATE SPATIAL INDEX IF NOT EXISTS $buildingsIdx ON $tableName (the_geom);
            -- List buildings that intersects with other buildings that have a greater area
            DROP TABLE IF EXISTS $tmpRelBuild;
            CREATE TABLE $tmpRelBuild AS SELECT s1.ID_WAY as PK_BUILDING, S2.ID_WAY as PK2_BUILDING FROM $tableName S1, $tableName S2 WHERE ST_AREA(S1.THE_GEOM) < ST_AREA(S2.THE_GEOM) AND S1.THE_GEOM && S2.THE_GEOM AND ST_DISTANCE(S1.THE_GEOM, S2.THE_GEOM) <= 0.1;
            
            -- Alter that small area buildings by removing shared area
            DROP TABLE IF EXISTS $tmpBuildTrunc;
            CREATE TABLE $tmpBuildTrunc AS SELECT PK_BUILDING, ST_DIFFERENCE(s1.the_geom, ST_BUFFER(ST_Collect(s2.the_geom), 0.1, 'join=mitre')) the_geom, s1.HEIGHT HEIGHT from $tmpRelBuild r, $tableName s1, $tableName s2 WHERE PK_BUILDING = S1.ID_WAY AND PK2_BUILDING = S2.ID_WAY  GROUP BY PK_BUILDING;
            
            -- Merge original buildings with altered buildings 
            DROP TABLE IF EXISTS $buildingsTable;
            CREATE TABLE IF NOT EXISTS $buildingsTable(PK INTEGER PRIMARY KEY, THE_GEOM GEOMETRY, HEIGHT real) AS SELECT s.id_way, ST_SETSRID(s.the_geom, '''+srid+'''), s.HEIGHT from $tableName s where id_way not in (select PK_BUILDING from $tmpBuildTrunc) UNION ALL select PK_BUILDING, ST_SETSRID(the_geom, $srid), HEIGHT from $tmpBuildTrunc WHERE NOT st_isempty(the_geom);
        ...
        sql.execute "CREATE SPATIAL INDEX IF NOT EXISTS ${buildingsTable}_INDEX ON $buildingsTable (THE_GEOM)"
    }

    if (!ignoreRoads) {
        // sql.execute("DROP TABLE IF EXISTS ROADS")
        sql.execute("CREATE IF NOT EXISTS TABLE ROADS (PK serial PRIMARY KEY, ID_WAY integer, THE_GEOM geometry, TYPE varchar, LV_D integer, LV_E integer,LV_N integer,HGV_D integer,HGV_E integer,HGV_N integer,LV_SPD_D integer,LV_SPD_E integer,LV_SPD_N integer,HGV_SPD_D integer, HGV_SPD_E integer,HGV_SPD_N integer, PVMT varchar(10));")
    ...
    if (!ignoreGround) {
        ...
        sql.execute("CREATE IF NOT EXISTS TABLE GROUND (PK serial PRIMARY KEY, ID_WAY int, THE_GEOM geometry, PRIORITY int, G double);")
