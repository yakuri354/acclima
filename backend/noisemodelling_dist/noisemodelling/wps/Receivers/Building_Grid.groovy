...
def exec(Connection connection, input) {
    ...
    if (input['fence'] != null) {
        fenceGeom = input['fence'] as Geometry
        // if (targetSrid != 0) {
        //     // Transform fence to the same coordinate system than the buildings & sources
        //     WKTReader wktReader = new WKTReader()
        //     fence = wktReader.read(input['fence'] as String)
        //     fenceGeom = ST_Transform.ST_Transform(connection, ST_SetSRID.setSRID(fence, 4326), targetSrid) // SUS maybe wrong srid? 4326
        // } else {
    ...
    // sql.eachRow("select * from tmp_receivers_lines") { row ->
